            ...
            for _, command in ipairs(registeredCommands) do
                for _, blacklistedCommand in pairs(blacklistedCommands) do
                    if (string.lower(command.name) == string.lower(blacklistedCommand) or
                        ...
                        TriggerServerEvent("Anticheat:Modder", "핵 사용 감지[인젝션 감지]", 
                            "[익스트림 안티치트]: " .. Config.Messages.CommandsTriggered:gsub("{COMMAND}", blacklistedCommand));
                    end
                end
            end
        end
...
if Config.Components.AntiESX then 
    ...
    AddEventHandler('esx:getSharedObject', function()
        TriggerServerEvent("Anticheat:ModderESX", "핵사용 감지 [ESX스크립트 감지]", 
                            "[익스트림 안티치트]: " .. Config.Messages.ESXTriggered);
    end)
end
 ]]--
                ...
                if #keyCombo == 1 then 
                    ...
                    if IsDisabledControlJustReleased(0, key1) then 
                        ...
                        if Config.KickForKeys then 
                            TriggerServerEvent("Anticheat:ModderNoKick", "금지된 사용키 사용 감지 [사용한 키: `" .. keyStr ..
                                "`]", "금지된 사용키 사용을 감지하였습니다", true);
                        else
                            TriggerServerEvent("Anticheat:ModderNoKick", "금지된 사용키 사용 감지 [사용한 키: `" .. keyStr ..
                                "`]", "금지된 사용키 사용을 감지하였습니다", false);
                        end
                    end
...
Citizen.CreateThread(function()
    while true do
        ...
        if ped == 1 then
            TriggerServerEvent("Anticheat:SpectateTrigger", "[익스트림 안티치트]: " .. Config.Messages.SpectateTriggered);
        end
    end
end)
